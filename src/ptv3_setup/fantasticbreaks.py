"""
Fantastic Breaks Classification Dataset for Pointcept (HDF5 direct reader)

Reads enriched HDF5 files directly — no format conversion needed.
Expected HDF5 structure:
    data:  float32 [B, N, 11]  (XYZ + 8 geometric features)
    label: int64   [B, 1]      (0=complete, 1=broken)

Place this file at:  src/ptv3/pointcept/datasets/fantasticbreaks.py
Then add to         src/ptv3/pointcept/datasets/__init__.py:
    from .fantasticbreaks import *

Author: Auto-generated for 3D-Segmentation-Restoration project.
"""
...
class FantasticBreaksClsDataset(Dataset):
    """Binary classification dataset: complete (0) vs broken (1).

    Reads *_data_enriched.h5 produced by compute_geometric_features.py.
    Each sample is [N, 11]:  cols 0-2 = XYZ, cols 3-10 = geometric features.

    The dict returned by __getitem__ contains:
        coord:    float32 [N, 3]   — XYZ (used by PTv3 for serialization)
        feat:     float32 [N, 8]   — geometric features
        category: int64   [1]      — class label
    """
    ...
    def __init__(
        ...
        with h5py.File(h5_path, "r") as f:
            self.points = f["data"][:].astype(np.float32)   # [B, N, 11]
            self.labels = f["label"][:].astype(np.int64)     # [B, 1]
        ...
        logger.info(
            f"FantasticBreaksClsDataset [{split}]: "
    ...
    def get_data(self, idx):
        ...
        sample = self.points[data_idx]  # [N, 11]
        coord = sample[:, :3].copy()    # [N, 3] — XYZ
        feat = sample[:, 3:].copy()     # [N, 8] — geometric features
        category = self.labels[data_idx].flatten().copy()  # [1]
